# rag agent
# seasrch KB
# check if retrievd docs are sufficient to answer query
# if yes, continue to generate answer
# if no, rewrite query to retrieve again till docs are good enough
# then generate answer
# after answer is generated, end agent loop, no answer grader
...
class BaseAgent:
    def __init__(self, args, tools) -> None:
        if args.use_hf_tgi:
            print('*************Using HF TGI LLM ENDPOINT**********')
            self.llm_endpoint = setup_llm(args)
        elif args.use_openai:
            print('*************Using OpenAI API**********')
            self.llm_endpoint = ChatOpenAI(model="gpt-4o-mini-2024-07-18", temperature=0)
...
class Retriever:
    ...
    def create(cls, tools_descriptions):
        return ToolNode(tools_descriptions) # note: tools_description = tools here
...
class RAGAgentwithLanggraph(BaseAgent):
    def __init__(self, args, tools):
        ...
        # Define Nodes
        document_grader = DocumentGrader(self.llm_endpoint, args.model_id)
